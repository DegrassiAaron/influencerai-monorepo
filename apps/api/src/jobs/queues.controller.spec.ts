...
describe('QueuesController', () => {
  it('aggregates job counts across queues', async () => {
    const moduleRef = await Test.createTestingModule({
      ...
      providers: [
        { provide: getQueueToken('content-generation'), useValue: createQueueMock({ active: 1, waiting: 2, failed: 3 }) },
        { provide: getQueueToken('lora-training'), useValue: createQueueMock({ active: 4, waiting: 5, failed: 6 }) },
        { provide: getQueueToken('video-generation'), useValue: createQueueMock({ active: 7, waiting: 8, failed: 9 }) },
      ],
    }).compile();
