...
const { createContentPlan, updateContentPlanApproval } = vi.hoisted(() => ({
  createContentPlan: vi.fn<[CreateContentPlanInput], Promise<ContentPlanJob>>(),
  updateContentPlanApproval: vi.fn<[UpdateContentPlanApprovalInput], Promise<unknown>>(),
})) as {
...
describe('ContentPlanWizard', () => {
  ...
  it('guides the user through steps and generates a preview', async () => {
    ...
    fireEvent.change(screen.getByLabelText(/persona summary/i), {
      target: { value: 'Energetic AI' },
    });
