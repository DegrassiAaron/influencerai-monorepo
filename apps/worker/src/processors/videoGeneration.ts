...
export function createVideoGenerationProcessor(deps: VideoGenerationDependencies) {
  ...
  const processor: Processor<VideoGenerationJobData, VideoGenerationResult, 'video-generation'> =
    async function process(job: VideoGenerationJob) {
      ...
      const durationSec =
        Number.isFinite(durationCandidate) && durationCandidate > 0 ? durationCandidate : 15;
      ...
      try {
        ...
        if (s3ClientInfo) {
          ...
          try {
            videoUrl = await s3.getSignedGetUrl(
              s3ClientInfo.client,
              s3ClientInfo.bucket,
              videoKey,
              7 * 24 * 3600
