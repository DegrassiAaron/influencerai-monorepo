...
# Ensure LF-friendly reading of .env and avoid expanding vars
...
Write-Host "Eseguo test di connessione (select 1) con Prisma..."

# Run prisma via pnpm workspace filter
# Use -- to prevent PowerShell from interpreting semicolons
& pnpm --filter @influencerai/api exec prisma db execute --script "select 1;" | Write-Output

Write-Host "OK: connessione riuscita"
